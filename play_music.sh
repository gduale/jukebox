...
do
  ...
  if [[ $code != "" ]] && [[ $player == "" ]]; then
    ...
    if [ $code == "d62caa1a" ];then 
     ...
     mplayer -really-quiet "mp3/Bateau-sur-leau.mp3" &
    fi
    
    if [ $code == "d674101e" ];then 
      current_code="d674101e"
     mplayer -really-quiet "mp3/Dans-sa-maison-un-grand-cerf.mp3" &
    fi
    
    if [ $code == "d6bfc61a" ];then 
      current_code="d6bfc61a"
     mplayer -really-quiet "mp3/GerardDaltonlasorciereGrabouilla.mp3" &
    fi
    
    if [ $code == "62262d95" ];then 
      current_code="62262d95"
     mplayer -really-quiet "mp3/LuisFonsi-Despacitoft.DaddyYankee.mp3" &
    fi
    
    if [ $code == "e695c41a" ];then 
      current_code="e695c41a"
     mplayer -really-quiet "mp3/PeppaPigFrancais-BingBongZoo.mp3" &
    fi
  fi
  sleep 1
done
